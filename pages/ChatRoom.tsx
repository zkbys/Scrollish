...
const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  const handleSend = async () => {
    ...
    if (isAiMode && quotedMessage) {
      ...
      // [新增] 立即添加一个"回复中..."的占位消息
      const loadingMsgId = `local-ai-loading-${Date.now()}`
      const loadingMsg: Comment = {
        id: loadingMsgId,
        post_id: postId,
      ...
      addLocalComment(postId, loadingMsg)
  ...
  return (
    <div
      ...
      <div
        ...
        <button
          disabled
          className="w-10 h-10 flex items-center justify-center bg-white/10 dark:bg-white/5 rounded-2xl border-2 border-white/10 dark:border-white/5 opacity-40 cursor-not-allowed shadow-lg"
          style={{ borderRadius: DROPLET_SHAPE }}>
          <span className="material-symbols-outlined text-white dark:text-white/60 text-[20px]">tune</span>
        </button>
      </div>
          ...
          <div className="flex gap-2 items-center">
            <div className="flex-1 relative flex items-center">
              <input
                ref={inputRef}
                disabled={!isAiMode}
                className={`w-full h-12 bg-transparent px-5 text-gray-900 dark:text-white text-[15px] outline-none border-none ring-0 ring-transparent focus:ring-0 focus:ring-transparent focus:outline-none placeholder:text-gray-400 dark:placeholder:text-white/20 font-medium ${!isAiMode ? 'cursor-not-allowed' : 'cursor-text'}`}

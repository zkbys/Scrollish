    ...
    return (
        <div className="flex flex-col items-center gap-1.5 relative select-none">
            ...
            <motion.button
                ...
                style={{ width: 'clamp(2.5rem,6.5vh,3rem)', height: 'clamp(2.5rem,6.5vh,3rem)' }}
            >
                <span
                    className={`material-symbols-outlined text-[clamp(26px,3.8vh,32px)] transition-all duration-300 ${isLiked ? 'text-orange-500 fill-[1] drop-shadow-[0_0_10px_rgba(249,115,22,0.4)]' : 'text-white/70 hover:text-white'}`}>
                    favorite
                </span>
            </motion.button>

            {count !== undefined && (
                <span className={`text-[clamp(10px,1.4vh,12px)] font-black drop-shadow-md transition-colors ${isLiked ? 'text-orange-400' : 'text-white/50'}`}>
                    {count}
                </span>
            )}
        </div>
